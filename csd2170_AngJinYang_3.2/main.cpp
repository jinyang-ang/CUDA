...
public:
	...
	struct HistoEqBuffer {
		uint32_t histoBin[256];
		float cdf[256];
	};

	vks::Buffer histoEqBuffer;
	...
	~VulkanExample()
	{
		...
		vkDestroyBuffer(device, histoEqBuffer.buffer, nullptr);
		vkFreeMemory(device, histoEqBuffer.memory, nullptr);
	...
	{
		///@William
		//textureColorMap.loadFromFile(getAssetPath() + "textures/jaguar.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT, VK_IMAGE_LAYOUT_GENERAL);
	...
	void buildComputeCommandBufferHistogram(VkBufferMemoryBarrier& bufferMemoryBarrier)
	{
		// Clear histogram data to 0
		vkCmdFillBuffer(compute.commandBuffer, histoEqBuffer.buffer, histoEqBuffer.descriptor.offset, histoEqBuffer.descriptor.range, 0);
		...
		bufferMemoryBarrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT; // For other synchronizations

		// Histogram computation
		vkCmdBindPipeline(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelines[0]);
		vkCmdBindDescriptorSets(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout, 0, 1, &compute.descriptorSet, 0, 0);
		vkCmdDispatch(compute.commandBuffer, (textureComputeTarget.width) / 16 + 1, (textureComputeTarget.height) / 16 + 1, 1);
	...
	void buildComputeCommandBufferCDFScan(VkBufferMemoryBarrier& bufferMemoryBarrier)
	{
		// CDF computation
	...
	void buildComputeCommandBufferApplyHisto()
	{
	...
	void buildComputeCommandBuffer() {
		...
		// Buffer memory barrier for histogram buffer
		...
		bufferMemoryBarrier.buffer = histoEqBuffer.buffer;
		bufferMemoryBarrier.offset = 0;
		bufferMemoryBarrier.size = histoEqBuffer.descriptor.range;

		buildComputeCommandBufferHistogram(bufferMemoryBarrier);
		buildComputeCommandBufferCDFScan(bufferMemoryBarrier);
		buildComputeCommandBufferApplyHisto();
	...
	void setupDescriptorPool()
	{
		std::vector<VkDescriptorPoolSize> poolSizes = {
			...
			// Compute pipelines use storage images for image reads and writes
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 2),
			// Compute pipelines use storage buffers for histogram equalization
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1),
		};
		VkDescriptorPoolCreateInfo descriptorPoolInfo = vks::initializers::descriptorPoolCreateInfo(poolSizes, 7);
	...
	void prepareCompute()
	{
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			...
			// Binding 2: Storage buffer for histogram equalization
		...
		bufferInfo.size = sizeof(HistoEqBuffer);
		bufferInfo.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;

		VK_CHECK_RESULT(vkCreateBuffer(device, &bufferInfo, nullptr, &histoEqBuffer.buffer));

		VkMemoryRequirements memReqs;
		vkGetBufferMemoryRequirements(device, histoEqBuffer.buffer, &memReqs);
		...
		allocInfo2.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

		VK_CHECK_RESULT(vkAllocateMemory(device, &allocInfo2, nullptr, &histoEqBuffer.memory));
		VK_CHECK_RESULT(vkBindBufferMemory(device, histoEqBuffer.buffer, histoEqBuffer.memory, 0));

		histoEqBuffer.descriptor.buffer = histoEqBuffer.buffer;
		histoEqBuffer.descriptor.offset = 0;
		histoEqBuffer.descriptor.range = sizeof(HistoEqBuffer);
		...
		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2, &histoEqBuffer.descriptor)
		};
		...
		// Build command buffers for each compute pipeline
	...
	void* mapHistoEqBuffer() {
		void* data;
		VK_CHECK_RESULT(vkMapMemory(device, histoEqBuffer.memory, 0, sizeof(HistoEqBuffer), 0, &data));
		return data;
	}
	void draw()
	{
		...
		//VkAppBase::saveScreenshot("output.ppm");
		//// Map histoEqBuffer memory to host visible memory
		//HistoEqBuffer* histoEqData = (HistoEqBuffer*)mapHistoEqBuffer();

		//// Open a file to write the histoBin and cdf values
		//std::ofstream outputFile("histoEqData.txt");
		//if (outputFile.is_open()) {
		//	// Write histoBin values
		//	outputFile << "histoBin:\n";
		//	for (uint32_t i = 0; i < 256; i+=4) {
		//		outputFile << histoEqData->histoBin[i] << " ";
		//		outputFile << histoEqData->histoBin[i+1] << " ";
		//		outputFile << histoEqData->histoBin[i+2] << " ";
		//		outputFile << histoEqData->histoBin[i+3] << " " << "\n";
		...
		//	outputFile << "cdf:\n";
		//	for (uint32_t i = 0; i < 256; i+=4) {
		//		outputFile << histoEqData->cdf[i] << " ";
		//		outputFile << histoEqData->cdf[i+1] << " ";
		//		outputFile << histoEqData->cdf[i+2] << " ";
		//		outputFile << histoEqData->cdf[i+3] << " " << "\n";

		//	}
		//	outputFile << "\n";

		//	outputFile.close();
		//	std::cout << "histoEqData written to histoEqData.txt\n";
		//}
		//else {
		//	std::cout << "Unable to open file to write histoEqData\n";
		//}

		//// Unmap the memory
		//vkUnmapMemory(device, histoEqBuffer.memory);
	}
	...
	void prepare()
	{
		...
		setupDescriptorSet();
		prepareGraphics();
		prepareCompute(); // Prepare compute pipelines and storage buffer
		buildCommandBuffers();
		prepared = true;
	}
